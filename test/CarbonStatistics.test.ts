...
test('it gets carbon statistics in default mode', () => {
    return sdk.carbonStatistics.get().then((data: any) => {
        expect(ApiResponse.isSuccessful(data)).toBe(true);
    });
});

test('it gets carbon statistics based on a specific day', () => {
    return sdk.carbonStatistics.getADay("2020-03-30").then((data: any) => {
        expect(ApiResponse.isSuccessful(data)).toBe(true);
        expect(data.dataInfo.period.start).toContain(
            "2020-03-30"
        );
    });
});
...
test('it gets carbon statistics based on a specific period', () => {
    ...
    return sdk.carbonStatistics.getAPeriod("2020-03-29").then((data: any) => {
        ...
        expect(data.dataInfo.period.end).toContain(
            today
        );
    });
});